...
func (d *discovery) setup(bootnodes []*peer.AddrInfo) error {
	...
	d.routingTable.PeerAdded = func(p peer.ID) {
		...
		d.srv.addToDialQueue(&info, PriorityRandDial)
	}
	...
	pb.RegisterDiscoveryServer(grpc.GrpcServer(), d)
	grpc.Serve()
...
func (d *discovery) findPeersCall(peerID peer.ID) ([]*peer.AddrInfo, error) {
	...
	clt := pb.NewDiscoveryClient(rawGrpcConn)

	resp, err := clt.FindPeers(context.Background(), &pb.FindPeersReq{Count: 16})
	if err != nil {
		return nil, err
	}
