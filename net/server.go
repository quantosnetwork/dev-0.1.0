...
const (
	...
	DefaultLibp2pPort      int    = 55655
	MinimumPeerConns       int64  = 1
	MinBootNodes           int    = 1
	PriorityReqDial        uint64 = 1
	PriorityRandDial       uint64 = 10
...
func getLibP2PKey() (crypto.PrivKey, error) {
	priv, _, err := crypto.GenerateKeyPair(crypto.Secp256k1, 256)
	return priv, err
}

func NewServer(logger hclog.Logger, config *Config) (*Server, error) {
	logger = logger.Named("quantos-network")
	key, err := getLibP2PKey()
	if err != nil {
		return nil, err
	}
	...
	ps, err := pubsub.NewGossipSub(
		context.Background(), host, pubsub.WithPeerOutboundQueueSize(peerOutboundBufferSize),
...
func (s *Server) Start() error {
	...
	if s.config.Chain.Bootnodes == nil {
		return errors.New("need at least 1 boot node to start")
	}
...
func (s *Server) Join(addr *peer.AddrInfo, timeout time.Duration) error {
	...
	s.addToDialQueue(addr, PriorityReqDial)
